...
export async function readActiveCoordinator(
  ...
  /* Identity, not reachability: the process answered, it just answered for
     another database. Never a CoordinatorUnreachableError, whose whole message
     is about waiting longer for a busy coordinator. */
  if (resolution.code === "identity") throw new Error(resolution.message)
      ...
      manifest
        ? [
            hasActiveGuiClient(manifest.key, root).then(async (active) => {
              ...
              if (!isCoordinatorProbeForManifest(manifest, (await probe(manifest, "quick")).probe)) return undefined

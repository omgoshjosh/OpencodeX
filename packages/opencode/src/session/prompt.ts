...
import { and, asc, eq, exists, gt, isNotNull, isNull } from "drizzle-orm"
import { SessionCommandTable, SessionExecutionTable, SessionTable } from "@opencode-ai/core/session/sql"
import { SessionReminders } from "./reminders"
import { SessionTools } from "./tools"
import { classifyProviderFailure, shouldAdvanceModelFallback } from "./model-fallback"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const liveQueue = (input: {
      ...
      const fence = (offer: OpencodeXClaudeDriver.LiveQueueOffer) =>
        and(
          ...
          eq(SessionCommandTable.adopted_by, input.commandID!),
          eq(SessionCommandTable.adopted_generation, input.claimGeneration!),
          eq(SessionCommandTable.offer_ordinal, offer.ordinal),
        )
      return {
        ...
        requeue: (offer: OpencodeXClaudeDriver.LiveQueueOffer) =>
          db
            .update(SessionCommandTable)
            .set({ adopted_by: null, adopted_generation: null, offer_ordinal: null, time_updated: Date.now() })
            .where(and(fence(offer), isNull(SessionCommandTable.offered_at)))
            .run()
            .pipe(Effect.asVoid, Effect.orDie),
        settle: (offer: OpencodeXClaudeDriver.LiveQueueOffer, error?: string) =>
          db
            .update(SessionCommandTable)
            .set({
            ...
            .where(and(fence(offer), isNotNull(SessionCommandTable.offered_at)))
            .run()
            .pipe(Effect.asVoid, Effect.orDie),
        failOffered: (error: string) =>
          db
            .update(SessionCommandTable)
            .set({ status: "failed", error, completed_at: Date.now(), time_updated: Date.now() })
            .where(
              and(
                ...
                eq(SessionCommandTable.adopted_by, input.commandID!),
                eq(SessionCommandTable.adopted_generation, input.claimGeneration!),
                isNotNull(SessionCommandTable.offered_at),
              ),
            )
            .run()
            .pipe(Effect.asVoid, Effect.orDie),
      } satisfies OpencodeXClaudeDriver.LiveQueue
    }
    ...
    const advanceExhaustedRoute = Effect.fn("SessionPrompt.advanceExhaustedRoute")(function* (input: {
      ...
      const turn = yield* sessions.messageWithChildren({ sessionID: input.sessionID, messageID: input.lastUser.id })
      const replayable = shouldAdvanceModelFallback(turn, input.lastUser.id)
      const attempted = SessionProviderExhaustion.attemptedRoutes(turn, input.lastUser.id)
      ...
      if (!next) {
        ...
        yield* sessions.updatePart({
          ...
          sessionID: input.sessionID,
          type: "text",
          text: SessionProviderExhaustion.exhaustionNotice({ ...failed, attempted }),
          synthetic: true,
      ...
      Effect.fn("SessionPrompt.run")(function* (input: { sessionID: SessionID; messageID?: MessageID }) {
        ...
        while (true) {
          ...
          if (outcome === "break") {
            ...
            // ending the turn, and if the chain is spent, say so - an assistant
            // row with no parts and no reason is indistinguishable from a
            // dropped write to every caller downstream.
            if (classifyProviderFailure(handle.message.error) === "exhausted") {

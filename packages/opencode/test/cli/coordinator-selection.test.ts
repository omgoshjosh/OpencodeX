...
describe("local coordinator database selection", () => {
  ...
  test("still discovers a legacy sidecar whose health carries no identity", async () => {
    // A GUI sidecar from a build predating the coordinatorKey health field
    // reports healthy with no identity. Rejecting it silently routes the CLI
    // to the default database while the GUI keeps its own - sessions invisible
    // across the two clients. Only an actively mismatching identity means
    // another database; readActiveCoordinator already applies exactly this
    // tolerance.
    await using tmp = await tmpdir()
    const database = path.join(tmp.path, "gui.db")
    const key = "gui-authority"
    ...
    const server = Bun.serve({
      port: 0,
      // Legacy shape: healthy, no coordinatorKey.
...
describe("attaching to a live coordinator", () => {
  ...
  test("refuses a coordinator that answers healthily for another database, however patient the probe", async () => {
    /* The retry makes this client wait longer for an answer; it must not make
       it less careful about whose answer it got. The manifest survives, because
       a live process still owns that database. */
    await withManifest(async ({ key, database }) => {
      let calls = 0
      const probe = createCoordinatorProber({
        ...
        fetch: injectedFetch((url, init) => {
          ...
          return calls < 3
            ? stalls(url, init)
            : Promise.resolve(Response.json({ healthy: true, version: "local", coordinatorKey: "another-database" }))
        }),
      })
      ...
      expect(calls).toBe(3)
      expect(String(error)).toContain("TUI coordinator process")
      expect(String(error)).toContain("answered for a different database; refusing to attach")
      expect(await Bun.file(coordinatorManifestPath(key)).exists()).toBe(true)
    })
  })

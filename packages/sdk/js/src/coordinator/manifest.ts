...
export type CoordinatorProbe =
  ...
  | { kind: "healthy"; version?: string; active?: boolean; ms: number }
  /** 2xx, parsed body, `healthy` was not `true`. The coordinator is up but says no. */
  | { kind: "unhealthy"; version?: string; active?: boolean; ms: number }
...
export async function fetchCoordinatorHealth(
  ...
  return { healthy: probe.kind === "healthy", version: probe.version, active: probe.active }
}
...
export type CoordinatorAttachment =
  ...
  | { action: "reclaim"; reason: "key_mismatch" | "database_mismatch" | "process_dead"; manifest: CoordinatorManifest }
  /** A live process holds the database and this client must not touch it. */
  | {
      action: "refuse"
      code: "version" | "unverifiable" | "foreign"
      message: string

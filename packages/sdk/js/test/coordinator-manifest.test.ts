...
describe("health probe", () => {
  ...
  test("matches a probe to the manifest database key, tolerating a coordinator too old to say", () => {
    const probe = (overrides: Record<string, unknown>) => ({ kind: "healthy" as const, ms: 1, ...overrides })
    expect(isCoordinatorProbeForManifest(manifest(), probe({ coordinatorKey: "abc123" }))).toBe(true)
    expect(isCoordinatorProbeForManifest(manifest(), probe({ coordinatorKey: "different" }))).toBe(false)
    /* No key at all is a pre-identity coordinator, not a foreign one. */
    expect(isCoordinatorProbeForManifest(manifest(), probe({}))).toBe(true)
    expect(isCoordinatorProbeForManifest(manifest(), { kind: "timeout", ms: 1 })).toBe(false)
  })

  test("the health probe reports both the answering identity and how long it took", async () => {
    const probe = await probeCoordinatorHealth(manifest(), {
      fetch: async () => healthyResponse({ healthy: true, version: "1.2.3", coordinatorKey: "abc123" }),
    })
    expect(probe).toMatchObject({ kind: "healthy", version: "1.2.3", coordinatorKey: "abc123" })
    if (probe.kind !== "healthy") throw new Error("unreachable")
...
describe("attachment resolution", () => {
  ...
  test("a healthy answer for another database is refused however patient the probe was", async () => {
    /* The retry and the identity check have to hold together: making the client
       wait longer for an answer must not make it less careful about whose
       answer it got. */
    let calls = 0
    const resolution = await resolveCoordinatorAttachment({
      manifest: manifest(),
      ...
      probe: prober((url, init) => {
        ...
        return calls < 3
          ? stalls(url, init)
          : Promise.resolve(healthyResponse({ healthy: true, version: "1.2.3", coordinatorKey: "other-database" }))
      }),
    })

    expect(calls).toBe(3)
    expect(resolution).toMatchObject({ action: "refuse", code: "identity" })
    if (resolution.action !== "refuse") throw new Error("unreachable")
    expect(resolution.message).toContain("answered for a different database; refusing to attach")
  })

  test("names the refusing client the way that client names itself", async () => {
    const resolution = await resolveCoordinatorAttachment({
      manifest: manifest(),
      key: "abc123",
      database: "/data/opencode.db",
      clientVersion: "1.2.3",
      mode: "decide",
      label: "TUI coordinator",
      processAlive: () => true,
      probe: prober(async () => healthyResponse({ healthy: true, version: "1.2.3", coordinatorKey: "other-database" })),
    })

    if (resolution.action !== "refuse") throw new Error("unreachable")
    expect(resolution.message).toBe(
      `TUI coordinator process ${process.pid} answered for a different database; refusing to attach`,
    )
  })

  test("reclaims a manifest whose answering process is gone and whose identity disagreed", async () => {
    const resolution = await resolveCoordinatorAttachment({
      manifest: manifest(),
      key: "abc123",
      database: "/data/opencode.db",
    ...
    expect(resolution).toMatchObject({ action: "reclaim", reason: "identity_mismatch" })
  })

  test("attaches to a coordinator too old to report an identity at all", async () => {
    /* Refusing the legacy shape would route this client to a different
       database while the old one keeps its own - two clients, two databases. */
    const resolution = await resolveCoordinatorAttachment({
      manifest: manifest(),
      key: "abc123",
      database: "/data/opencode.db",
      clientVersion: "1.2.3",
      mode: "decide",
      probe: prober(async () => healthyResponse({ healthy: true, version: "1.2.3" })),
    })

    expect(resolution.action).toBe("attach")
  })

  test("carries the compatibility reason so a caller can tell a mismatch from a stale manifest", async () => {
    /* The GUI offers the human an "attach anyway" override for a plain version
       mismatch and for nothing else, so the reason has to survive the decision. */
    expect(
      await resolveCoordinatorAttachment({
        manifest: manifest({ serverVersion: undefined }),
        key: "abc123",
        database: "/data/opencode.db",
        clientVersion: "1.2.3",
        mode: "decide",
    ...
    ).toMatchObject({ action: "refuse", code: "version", reason: "legacy" })
    expect(
      await resolveCoordinatorAttachment({
    ...
    ).toMatchObject({ action: "refuse", code: "version", reason: "mismatch" })
  })
